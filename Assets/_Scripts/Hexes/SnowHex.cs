...
{
   public class SnowHex: BaseHexType
    {
        public SnowHex(Position position, int layer):base(position,layer)
        {
        }
        ...
        public override void OnLeaveHex(Position nextHex)
        {
          //  StartCoroutine(HideGrass());
            OnChangeHex(new IceHex(Position,Layer),HideSnow);
            base.OnLeaveHex(nextHex);
        }

        public IEnumerator HideSnow()
        {
            Debug.LogError("XXXX");
            var renderer = Instance.transform.GetComponent<MeshRenderer>();
